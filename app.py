...
if st.sidebar.button("Executar Análise"):
    ...
    if not lista_acoes or any(len(acao) != 5 for acao in lista_acoes):
        st.warning("Erro: Certifique-se de inserir códigos de ações válidos, separados por vírgulas (ex: PETR4,VALE3).")
    else:
        try:
            ...
            try:
                ...
                noticias = recuperar_noticias_financeiras(NewsApiClient.get_everything, lista_acoes)
                ...
                st.warning(f"Houve um erro na análise de sentimentos: {e}")
        except Exception as e:
            st.warning(f"Erro: Houve um erro ao otimizar o portfolio: {e}")
